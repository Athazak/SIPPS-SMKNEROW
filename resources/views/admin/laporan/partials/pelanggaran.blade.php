...
<div class="bg-white shadow rounded-xl px-4 py-3 mb-3 border border-gray-100">
    <div class="flex flex-col sm:flex-row justify-between items-center gap-4 mb-2">
        ...
        <div class="flex sm:flex-row sm:items-center gap-2">
            ...
            <a href="{{ route('admin.laporan.cetak', array_merge(request()->all(), ['tab' => 'pelanggaran', 'format' => 'excel'])) }}"
                class="px-3 py-2 text-sm rounded-lg bg-[#22A447] hover:bg-[#1c8a3c] text-white transition">
                Export Excel
            </a>
        </div>
    </div>
    ...
    <div class="mt-4">
        {{ $catatanPelanggarans->appends(['tab' => 'pelanggaran'])->links('vendor.pagination.simple-modern') }}
    </div>
</div>