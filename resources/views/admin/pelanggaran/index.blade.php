<x-app-layout>
    ...
    <div class="py-1"
        x-data="{ openTambah: false, openEdit: false, openDelete: false, deleteId: '', selected: { id: '', jenis_pelanggaran: '', bentuk: '', skor: '' } }">
        <div class="max-w-7xl mx-auto px-3 lg:px-4">
            ...
            <div class="rounded-2xl shadow px-4 py-4 mb-3 flex items-center bg-white text-white">
                ...
                <div class="flex flex-col">
                    ...
                    <p class="text-sm text-gray-600 mt-1">
                        Halaman ini digunakan untuk mengelola jenis pelanggaran, bentuk, dan skor.
                    </p>
                </div>
            </div>
                ...
                <form method="GET" class="w-full sm:max-w-xs">
                    <div class="relative w-full">
                        <input type="text" name="search" value="{{ request('search') }}"
                            placeholder="Cari jenis, bentuk, skor..."
                    ...
                    <table class="min-w-full text-sm text-left">
                        ...
                        <thead>
                            <tr class="bg-[#F4F5FF] dark:bg-gray-700 text-[#2A166F] dark:text-gray-200 border-b">
                                ...
                                <th class="px-4 py-3 font-semibold">Jenis Pelanggaran</th>
                        ...
                        <tbody>
                            @forelse ($pelanggarans as $pelanggaran)
                                <tr class="border-b hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                                    ...
                                    <td class="px-4 py-2 font-medium">{{ $pelanggaran->jenis_pelanggaran }}</td>
                                    <td class="px-4 py-2 text-gray-700">{{ $pelanggaran->bentuk }}</td>
                                    <td class="px-4 py-2">
                                        <span
                                    ...
                                    <td class="px-4 py-2">
                                        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-center gap-2">
                                            <!-- EDIT -->
                                            <button
                                                @click=" openEdit = true; selected.id = '{{ $pelanggaran->id }}'; selected.jenis_pelanggaran = '{{ $pelanggaran->jenis_pelanggaran }}'; selected.bentuk = '{{ $pelanggaran->bentuk }}'; selected.skor = '{{ $pelanggaran->skor }}';"
                                            ...
                                            <!-- DELETE -->
                                            <button @click="openDelete = true; deleteId = '{{ $pelanggaran->id }}'"
            ...
            <x-modal title="Konfirmasi Hapus" show="openDelete">
                <form :action="`{{ url('admin/pelanggaran') }}/${deleteId}`" method="POST">
                    @csrf
                    @method('DELETE')
